...
def pipeline(config):
    ...
    def run_gridsearch(X_train, y_train, FE):
        ...
        param_grid = {
                        'model__n_estimators': [100, 200],  # More typical values for RandomForest
                        'model__max_depth': [3, 5, None],  # None means no limit on the depth
                        'model__min_samples_split': [2, 10],  # Minimum number of samples required to split an internal node
                        'model__min_samples_leaf': [1, 4]  # The minimum number of samples required to be at a leaf node
                    }
    ...
    def mlflow_logging(
       best_model_params, best_model, best_mse, best_mae, best_r2, best_rmse,  X):
        ...
        experiment_name = "Timeseries forecast"
